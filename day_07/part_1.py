...
def run_program(opcodes, *args):
    if args is not None:
        input_iterator = iter(args)
    index = 0
    while True:
        ...
        match current_op % 100:
            ...
            case 3:
                opcodes[operands[0]] = next(input_iterator)
                index += 2
...
def amplifiers_output(phase_settings):
    ...
    for phase in phase_settings:
        last_output = next(
            run_program([op for op in start_opcodes], phase, last_output)
        )
    return last_output


print(max(amplifiers_output(p) for p in permutations(range(5))))
